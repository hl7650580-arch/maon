...
export default async function TrackingPage() {
  const [residentsRaw, trackingRaw, photosRaw] = await Promise.all([
    sql`SELECT id, name, housing_group, employment_group FROM residents WHERE is_active = 1 ORDER BY name`,
  ...
  const residents = residentsRaw as { id: number; name: string; housing_group: string; employment_group: string }[];
